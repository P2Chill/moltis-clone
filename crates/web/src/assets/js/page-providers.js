...
// Key used when WRITING a new override — exact stripped model ID (strips provider:: prefix).
// Key used when WRITING a new override — always the exact stripped model ID,
...
// Grey pill = no explicit entry (backend broad keys may still apply at runtime).
...
// Cycle trust pill and persist. MCP cycles: null→false→true→null. Sandbox: null→true→false→null.
function cycleTrustPill(modelId, field) {
	...
	params[field] = next; // null clears the field on the backend
